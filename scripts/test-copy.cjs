...
function setup(mode, noSelection=false) {
  ...
  return {buttons,statuses,disclosure,read:()=>({copied,focused,scrolled,selected})};
}
...
for(const mode of ['denied','missing'])test(`${mode} clipboard makes the manual fallback reachable`,async()=>{
  ...
  assert(state.focused && state.scrolled && state.selected);assert(x.disclosure.open);assert.equal(state.copied,undefined);assert.match(x.statuses['copy-status'].textContent,/download/);
});
